...
const PaymentResponse = () => {
    ...
    const paylater = orderStatus === 'pay later';
    ...
    return (
        <div className={`max-w-md mx-auto mt-48 mb-48 p-4 rounded-lg ${isSuccess ? 'bg-green-100' : 'bg-gray-100'} shadow-lg`}>
            ...
            <p className="text-sm">Order Status: <span className={`font-medium uppercase ${(isSuccess || paylater) ? 'text-green-600' : 'text-red-600'}`}>{orderStatus}</span></p>
            <p className="text-sm mt-3">
                {!isAwaited && <strong>{(isSuccess || paylater )? 'Please check your email & spam inbox for the order details.' : 'Something went wrong. Please try to register again and if you face the same issue please contact support@mioc.org.om'}
